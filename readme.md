...
# 논리연산자와 비교연산자,논리데이터
## 논리데이터 
* Boolean 참(True) 또는 거짓 (False)두 가지 값 중 하나의 데이터형식 조건문,논리연산자 , 논리적 판단 등에 사용합니다.
## 비교연산자
* 비교 연산자 
두 값 이상을 비교하여 그 결과를 논리데이터로 반환하는 연산자  * === 이건 완벽하게 같은가?
일치연산자(x==y)(x===y)
부등연산자(x!=y)(x!==y)  x와y 값이 다른가?
대소 비교 연산자 (x<y) (x>y) (x<=y)(x>=y)
논리연산자를 이용한 복합비교연산자
(x>5&&y<30)(x===10 || y===15)            →  && = and     ||=or
## 논리연산자
* 비교연산 또는 논리데이터를 좌우 두값으로 두고 논리적으로 비교하는 연산자이다
AND(&&) OR(||)NOT(!)

- AND(&&):양쪽 피연산자가 모두 참일 떄 결과는 참
